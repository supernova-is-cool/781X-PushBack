...
void autons::skills() {
  ...
  // Clean reverse (straight + damped)
  bot.tank(-55, -55);
  // waitUntil([] { return std::abs(bot.m_config.sensors.imu.get_pitch()) > 10;
  // }, 0, 1200);
  // waitUntil([] { return std::abs(bot.m_config.sensors.imu.get_pitch()) < 3;
  // }, 0, 1200);
  pros::delay(1200);
  bot.tank(0, 0);
  ...
  bot.moveToLine((middleGoal + Pose::fromPolar(10, -45)).withTheta(45), 1500,
...
//   bot.intake.goToSlow();
  ...
  bot.moveToPose(farLeftMlAlignTarget, 2000,
                 {
                     .horizontalDrift = 2,
                     .maxSpeed = 80,
                     .minSpeed = 40,
  ...
  bot.swingToHeading(BLUE_STATION, lemlib::DriveSide::LEFT, 2000,
                     {.direction = lemlib::AngularDirection::CW_CLOCKWISE,
                      ...
                      .minSpeed = 30});
  bot.waitUntilDone();
  // bot.turnToPoint(farLongGoalRight, 300, {.forwards = false});
...
//   bot.intake.goToTOP();
  ...
  pros::delay(1200);
  bot.tank(0, 0);
  bot.turnToHeading(BLUE_STATION, 200);
  bot.waitUntilDone();
...
//   bot.intake.goToTOP();
  ...
  bot.moveToX(closeLeftMatchLoader.x, 2000,
              {.maxSpeed = 45, .minSpeed = 10, .targetHeading = RED_STATION});
  bot.waitUntilDone();
...
//   bot.intake.goToTOP();
  ...
  */
  ...
  bot.moveToPose(
      {-2.5 * TILE, -TILE - 2, -20}, 2000,
      {.lead = 0.35, .maxSpeed = 90, .minSpeed = 60, .earlyExitRange = 7});
  bot.waitUntilDone();

  // bot.turnToHeading(-20, 1000);
  // bot.waitUntilDone(); // bot.matchLoader.extend();
  bot.park.extend();
