...
static const char
    *btn_map[ts::SELECTOR_ROWS * ts::SELECTOR_COLS + ts::SELECTOR_COLS + 1] =
        {}; // static as to not be deleted

std::string read_saved_auton() {
  std::ifstream auton_file(ts::SELECTOR_AUTON_FILE_PATH);
  if (!auton_file || !auton_file.is_open())
    return ts::SELECTOR_NO_AUTON_TEXT;

  std::string line;
  if (!std::getline(auton_file, line))
    return ts::SELECTOR_NO_AUTON_TEXT;
...
void ts::selector::register_auton(ts::auton a) {
  ...
  if (selector_instance)
    selector_instance.get()->refresh_selector();
}
...
void ts::selector::handle_events(lv_event_t *e) {
  ...
  if (obj == selector->l_run_selected_auton_button) {
    ...
    uint32_t btn_id = lv_buttonmatrix_get_selected_button(
        obj); // get the ID of the pressed/released button
    const char *btn_text = lv_buttonmatrix_get_button_text(obj, btn_id);
    if (std::string(btn_text) == SELECTOR_NO_AUTON_TEXT)
      return;
...
ts::selector *ts::selector::get() {
  if (!selector_instance)
    selector_instance = std::unique_ptr<ts::selector>(new selector());
...
ts::selector::selector() {
  ...
  lv_obj_add_event_cb(btnm, selector::handle_events, LV_EVENT_VALUE_CHANGED,
                      nullptr);
...
bool ts::selector::is_auton_selected() {
  if (a_selected_auton == SELECTOR_EMPTY_STRING)
    return false;
  if (a_selected_auton == SELECTOR_NO_AUTON_TEXT)
    return false;
  if (a_selected_auton == SELECTOR_INVALID_AUTON_TEXT)
    return false;
  return true;
}
...
void ts::selector::run_selected_auton() {
  ...
  for (ts::auton auton : registry_internal) {
    if (auton.name == a_selected_auton)
...
std::string ts::selector::get_selected_auton_name() { return a_selected_auton; }
...
extern "C" void ts_display_selector() { ts::selector::get()->display(); }

extern "C" void ts_hide_selector() { ts::selector::get()->hide(); }
...
extern "C" const char *ts_get_selected_auton_name() {
  static std::string selected_auton_name =
      ts::selector::get()->get_selected_auton_name();
...
extern "C" void ts_cycle_autons() { ts::selector::get()->cycle_autons(); }