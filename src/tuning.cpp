// Copied from
// https://github.com/darkMatter781x/HighStakes/blob/main/src/tuning.cpp
...
constexpr float celsiusToFahrenheit(float c) { return c * 9 / 5 + 32; }
...
void print(const TUNING_MODE mode, bool key = true) {
  ...
  for (const auto &temp : temps)
    sum += temp;
  float average = celsiusToFahrenheit(sum / temps.size());
  printf("motor temp: %f°F\n", average);
}

bool isTuningVar = false;

} // namespace

bool isTuning() { return isTuningVar; }

void tuningCLI() {
  isTuningVar = true;

  static TUNING_MODE mode = LATERAL;
  ...
  while (1) {
    ...
    try {
      ...
      if (command == "s" || command == "set") {
        ...
        switch (mode) {
        case LATERAL:
          bot.moveToY(multiplier * lateralDist, timeout);
          break;
        case ANGULAR:
